SELECT s.student_number AS "Student Identifier"
...
LEFT JOIN (
  SELECT studentid AS race_studentid
        ,LISTAGG(racecd, ',') WITHIN GROUP (ORDER BY racecd) AS racecd_agg
  FROM studentrace
  GROUP BY studentid
 ) rc
  ON s.id = rc.race_studentid
LEFT JOIN (
    SELECT c.contacts_studentdcid
    ...
    FROM
        (
         SELECT sca.studentdcid AS contacts_studentdcid
               ...
               ,ROW_NUMBER() OVER(
                  PARTITION BY sca.studentdcid
                    ORDER BY sca.contactpriorityorder) AS contacts_rn
    ...
    LEFT JOIN (
        SELECT ppna.personid AS p1_personid
              ,ppna.phonenumberasentered AS p1_phonenumberasentered
              ,ROW_NUMBER() OVER(
                 PARTITION BY ppna.personid, c.code
                   ORDER BY ppna.phonenumberpriorityorder) AS p1_filteredpriorityorder
        FROM personphonenumberassoc ppna
        JOIN codeset c
          ON ppna.phonetypecodesetid = c.codesetid
         AND c.code = 'Mobile'
      ) p1
      ON c.contacts_personid = p1.p1_personid
     AND p1.p1_filteredpriorityorder = 1
    LEFT JOIN (
        SELECT ppna.personid AS p2_personid
              ,ppna.phonenumberasentered AS p2_phonenumberasentered
              ,ROW_NUMBER() OVER(
                 PARTITION BY ppna.personid, c.code
                   ORDER BY ppna.phonenumberpriorityorder) AS p2_filteredpriorityorder
        FROM personphonenumberassoc ppna
        JOIN codeset c
          ON ppna.phonetypecodesetid = c.codesetid
         AND c.code = 'Home'
      ) p2
      ON c.contacts_personid = p2.p2_personid
     AND p2.p2_filteredpriorityorder = 1
    LEFT JOIN (
        SELECT ppna.personid AS p3_personid
              ,ppna.phonenumberasentered AS p3_phonenumberasentered
              ,ROW_NUMBER() OVER(
                 PARTITION BY ppna.personid, c.code
                   ORDER BY ppna.phonenumberpriorityorder) AS p3_filteredpriorityorder
        FROM personphonenumberassoc ppna
        JOIN codeset c
          ON ppna.phonetypecodesetid = c.codesetid
         AND c.code = 'Work'
      ) p3
      ON c.contacts_personid = p3.p3_personid
     AND p3.p3_filteredpriorityorder = 1
    LEFT JOIN (
        SELECT peaa.personid AS emails_personid
              ,e.emailaddress AS emails_emailaddress
              ,c.code AS emailtype
              ,ROW_NUMBER() OVER(
                 PARTITION BY peaa.personid, c.code
                   ORDER BY peaa.emailaddresspriorityorder) AS emails_filteredpriorityorder
        FROM personemailaddressassoc peaa
        JOIN emailaddress e
          ON peaa.emailaddressid = e.emailaddressid
        JOIN codeset c
          ON peaa.emailtypecodesetid = c.codesetid
      ) em
      ON c.contacts_personid = em.emails_personid
     AND em.emails_filteredpriorityorder = 1
     AND em.emailtype = 'Current'
    WHERE c.contacts_rn = 1
   ) hoh
  ON s.dcid = hoh.contacts_studentdcid
LEFT JOIN (
    SELECT c.contacts_studentdcid
          ,c.contacts_firstname
          ,c.contacts_lastname
    ...
    LEFT JOIN (
     ...
     AND em.emails_filteredpriorityorder = 1
     AND em.emailtype = 'Current'
    WHERE c.contacts_rn = 2
   ) c2
  ON s.dcid = c2.contacts_studentdcid
