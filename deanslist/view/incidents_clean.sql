...
SELECT
  ...
  send_alert,
  hearing_flag,
  CAST(addl_reqs AS NVARCHAR(2048)) AS addl_reqs,
  CAST(admin_summary AS NVARCHAR(MAX)) AS admin_summary,
  CAST(category AS NVARCHAR(256)) AS category,
  CAST(context AS NVARCHAR(MAX)) AS context,
  CAST(family_meeting_notes AS NVARCHAR(MAX)) AS family_meeting_notes,
  CAST(gender AS NVARCHAR(2)) AS gender,
  CAST(grade_level_short AS NVARCHAR(8)) AS grade_level_short,
  CAST(homeroom_name AS NVARCHAR(64)) AS homeroom_name,
  CAST(infraction AS NVARCHAR(256)) AS infraction,
  CAST([location] AS NVARCHAR(128)) AS [location],
  CAST(reported_details AS NVARCHAR(MAX)) AS reported_details,
  CAST(return_period AS NVARCHAR(8)) AS return_period,
  ...
  CAST(update_middle AS NVARCHAR(16)) AS update_middle,
