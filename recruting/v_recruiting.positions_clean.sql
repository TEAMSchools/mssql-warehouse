...
CREATE OR ALTER VIEW recruiting.positions_clean AS
...
SELECT p.position_number
      ...
      ,p.created_date
      ,p.date_filled
      ,CASE WHEN p.n = 4 THEN PARSENAME(p.position_name_splitter,4) ELSE 'Invalid position_name Format' END AS recruiter
      ,CASE WHEN p.n = 4 THEN PARSENAME(p.position_name_splitter,3) ELSE 'Invalid position_name Format' END AS location
      ,CASE WHEN p.n = 4 THEN PARSENAME(p.position_name_splitter,2) ELSE 'Invalid position_name Format' END AS role_short
      ,CASE WHEN p.n = 4 THEN PARSENAME(p.position_name_splitter,1) ELSE 'Invalid position_name Format' END AS recruiing_year
      ,p.position_count

FROM position_parse p