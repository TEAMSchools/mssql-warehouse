...
FROM
  ...
  LEFT JOIN etr_pivot e ON c.df_employee_number = e.df_employee_number
  AND e.academic_year = gabby.utilities.GLOBAL_ACADEMIC_YEAR ()
  LEFT JOIN etr_pivot p ON c.df_employee_number = p.df_employee_number
  AND p.academic_year = gabby.utilities.GLOBAL_ACADEMIC_YEAR () - 1
  LEFT JOIN elementary_grade g ON c.df_employee_number = g.employee_number
  LEFT JOIN gabby.surveys.intent_to_return_survey_detail i ON c.df_employee_number = i.respondent_df_employee_number
