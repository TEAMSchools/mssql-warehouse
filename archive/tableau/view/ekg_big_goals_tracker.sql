...
WITH
  ...
  lit_growth AS (
    SELECT
      ...
      CASE
        WHEN MAX(
          CASE
            WHEN rn_curr = 1 THEN gleq
          END
        ) - MAX(
          CASE
            WHEN rn_base = 1 THEN gleq
          END
        ) >= 1 THEN 1.0
        WHEN MAX(
          CASE
  ...
  suspensions AS (
    SELECT
      ...
      MAX(
        CASE
          WHEN sub.att_code_group = 'ISS' THEN sub.n_streaks
        END
      ) AS [ISS],
      MAX(
        CASE
          WHEN sub.att_code_group = 'T' THEN sub.n_days
        END
      ) AS n_days_tardy
    FROM
      (
  ...
  student_attrition AS (
    SELECT
      d.student_number AS denominator_student_number,
  ...
  teacher_attrition AS (
    SELECT
      ...
      ISNULL(
        CAST(school_level AS VARCHAR(5)),
        'All'
      ) AS school_level,
      ...
      AVG(
        CAST(is_attrition_other AS FLOAT)
    ...
    FROM
      (
        SELECT
          ...
          is_attrition,
          CASE
            WHEN status_reason = 'Termination' THEN is_attrition
            ELSE 0
          END AS is_attrition_termination,
          CASE
            WHEN status_reason = 'Resignation' THEN is_attrition
            ELSE 0
  ...
  tntp AS (
    SELECT
      ...
      CASE
        WHEN [Learning Environment Score] >= [Learning Environment Score: Top-Quartile] THEN 1
        ELSE 0
      END AS is_top_quartile_learning_environment_score,
      ROW_NUMBER() OVER (
        PARTITION BY
          academic_year,
          reporting_schoolid,
  ...
  manager_survey AS (
    ...
    FROM
      (
        SELECT
          subject_primary_site_schoolid AS reporting_schoolid,
  ...
  ekg_walkthrough AS (
    ...
    FROM
      (
        ...
        MAX(pct_of_classrooms_proficient) FOR rubric_strand_field IN (
          [threecsaverage],
          [overallaverage]
        )
      ) AS p
  ...
  student_level_rollup_y1 AS (
    SELECT
      ...
      ISNULL(
        CAST(sub.school_level AS VARCHAR(5)),
        'All'
      ) AS school_level,
      ISNULL(sub.reporting_schoolid, 0) AS reporting_schoolid,
      ISNULL(
        CAST(sub.grade_level AS VARCHAR(5)),
        'All'
      ) AS grade_level
      /* student-level percentages */
,
      CAST(
        AVG(sub.is_free_or_reduced) AS FLOAT
      ...
      CAST(
        AVG(sub.parcc_ela_proficient) AS FLOAT
      ) AS parcc_ela_proficient_pct,
      CAST(
        AVG(sub.parcc_math_proficient) AS FLOAT
      ) AS parcc_math_proficient_pct,
      CAST(
        AVG(sub.parcc_ela_proficient_iep) AS FLOAT
      ) AS parcc_ela_proficient_iep_pct,
      CAST(
        AVG(sub.parcc_math_proficient_iep) AS FLOAT
      ) AS parcc_math_proficient_iep_pct,
      CAST(
        AVG(sub.parcc_ela_approaching_iep) AS FLOAT
      ) AS parcc_ela_approaching_iep_pct,
      CAST(
        AVG(sub.parcc_math_approaching_iep) AS FLOAT
      ) AS parcc_math_approaching_iep_pct,
      CAST(
        AVG(sub.module_ela_is_mastery) AS FLOAT
      ) AS module_ela_mastery_pct,
      CAST(
        AVG(sub.module_math_is_mastery) AS FLOAT
      ...
      CAST(
        AVG(sub.lit_making_1yr_growth) AS FLOAT
      ) AS lit_making_1yr_growth_pct,
      CAST(
        AVG(sub.is_student_attrition) AS FLOAT
...
,
    ...
    FROM
      ...
      LEFT JOIN q12 ON slr.reporting_schoolid = q12.reporting_schoolid
      ...
      LEFT JOIN hsr ON slr.reporting_schoolid = hsr.reporting_schoolid
      AND slr.region = hsr.region
      AND slr.school_level = hsr.school_level
      AND slr.academic_year = hsr.academic_year
      LEFT JOIN tntp ON slr.reporting_schoolid = tntp.reporting_schoolid
