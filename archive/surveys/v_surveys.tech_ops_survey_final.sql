...
WITH
    to_survey AS (
        ...
        FROM
            gabby.surveys.tech_ops_survey sur
            LEFT JOIN gabby.adsi.user_attributes_static adsi ON sur.please_confirm_your_email_address_please_use_your_kippnj_org_address_for_tracking_purposes_only_ = adsi.mail
            LEFT JOIN gabby.dayforce.staff_roster roster ON adsi.idautopersonalternateid = roster.adp_associate_id
    ),
    to_long AS (
        SELECT
            associate_id,
