...
FROM
  response_rollup rr
  JOIN gabby.illuminate_public.students s ON rr.student_id = s.student_id
  LEFT JOIN gabby.illuminate_dna_assessments.performance_band_lookup_static pbl ON rr.performance_band_set_id = pbl.performance_band_set_id
  AND rr.percent_correct
  ...
  AND co.rn_year = 1
  LEFT JOIN gabby.reporting.reporting_terms rta ON rr.administered_at
  --BETWEEN rta.[start_date] AND rta.end_date
  AND co.schoolid = rta.schoolid
  AND rta.identifier = 'RT'
  AND rta._fivetran_deleted = 0
  LEFT JOIN gabby.reporting.reporting_terms rtt ON rr.date_taken
  --BETWEEN rtt.[start_date] AND rtt.end_date
