  ...
  FROM
      (
       SELECT m.student_id             
             ...
             ,CASE 
               WHEN COUNT(m.student_id) OVER(PARTITION BY m.student_id, m.measurement_scale) < 4 THEN NULL 
               ELSE m.test_duration_minutes
              END AS student_testdurationminutes
                
             ,LAG(m.academic_year, 1) OVER(PARTITION BY m.student_id, m.measurement_scale ORDER BY m.test_start_date ASC) AS prev_academic_year
             ,LAG(m.term, 1) OVER(PARTITION BY m.student_id, m.measurement_scale ORDER BY m.test_start_date ASC) AS prev_term
             ,LAG(m.test_ritscore, 1) OVER(PARTITION BY m.student_id, m.measurement_scale ORDER BY m.test_start_date ASC) AS prev_rit
             ,LAG(m.percentile_2015_norms, 1) OVER(PARTITION BY m.student_id, m.measurement_scale ORDER BY m.test_start_date ASC) AS prev_npr

             ,LEAD(m.academic_year, 1) OVER(PARTITION BY m.student_id, m.measurement_scale ORDER BY m.test_start_date ASC) AS next_academic_year
             ,LEAD(m.term, 1) OVER(PARTITION BY m.student_id, m.measurement_scale ORDER BY m.test_start_date ASC) AS next_term
             ,LEAD(m.test_ritscore, 1) OVER(PARTITION BY m.student_id, m.measurement_scale ORDER BY m.test_start_date ASC) AS next_rit
             ,LEAD(m.percentile_2015_norms, 1) OVER(PARTITION BY m.student_id, m.measurement_scale ORDER BY m.test_start_date ASC) AS next_npr
     ...
     FROM
         (
          SELECT sub.student_id
                ...
                ,sub.prev_npr_change
                ,sub.next_npr_change
                ,AVG(prev_npr_change) OVER(PARTITION BY grade_level, measurement_scale, term, prev_term) AS mean_prev_npr_change
                ,STDEV(prev_npr_change) OVER(PARTITION BY grade_level, measurement_scale, term, prev_term) AS stdev_prev_npr_change
                ,AVG(next_npr_change) OVER(PARTITION BY grade_level, measurement_scale, term, next_term) AS mean_next_npr_change
                ,STDEV(next_npr_change) OVER(PARTITION BY grade_level, measurement_scale, term, next_term) AS stdev_next_npr_change
          FROM
              (
