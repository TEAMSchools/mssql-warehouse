CREATE
OR ALTER VIEW fast.student_data_long AS WITH student_data AS (
    SELECT
        _file,
        _line,
        student_id,
        ...
        (
            CASE WHEN CHARINDEX('ELAReading', _file) > 0 THEN 'ELA' WHEN CHARINDEX('Mathematics', _file) > 0 THEN 'MATH' ELSE '' END
        ) + ' ' + CAST(
            (
                CASE WHEN CHARINDEX('Grade3', _file) > 0 THEN 3 WHEN CHARINDEX('Grade4', _file) > 0 THEN 4 WHEN CHARINDEX('Grade5', _file) > 0 THEN 5 WHEN CHARINDEX('Grade6', _file) > 0 THEN 6 WHEN CHARINDEX('Grade7', _file) > 0 THEN 7 WHEN CHARINDEX('Grade8', _file) > 0 THEN 8 ELSE 0 END
            ) AS VARCHAR(10)
        ) AS fast_test,
        CASE WHEN CHARINDEX('ELAReading', _file) > 0 THEN 'Reading' WHEN CHARINDEX('Math', _file) > 0 THEN 'Math' END AS fast_subject
    FROM
        fast.student_data
),
domain_mastery AS (
