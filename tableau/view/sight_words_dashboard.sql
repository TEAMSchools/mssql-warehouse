...
FROM
  ...
  INNER JOIN gabby.illuminate_dna_repositories.repositories AS r ON (
    ds.code_id = r.code_scope_id
  ...
  INNER JOIN gabby.illuminate_dna_repositories.repository_grade_levels AS g ON (
    r.repository_id = g.repository_id
  )
  INNER JOIN gabby.powerschool.cohort_identifiers_static AS co ON (
    g.grade_level_id = (co.grade_level + 1)
    AND rt.academic_year = co.academic_year
    AND co.is_enrolled_recent = 1
    AND co.rn_year = 1
  )
  LEFT JOIN gabby.illuminate_dna_repositories.sight_words_data AS sw ON (
    co.student_number = sw.local_student_id
...
FROM
  ...
  INNER JOIN gabby.illuminate_dna_repositories.sight_words_data AS sw ON (
    co.student_number = sw.local_student_id
    AND r.repository_id = sw.repository_id
    AND f.[label] = sw.[label]
  )
