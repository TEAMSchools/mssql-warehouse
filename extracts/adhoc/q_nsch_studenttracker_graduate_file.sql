SELECT 'PH3' AS ColA
      ...
      ,'P' AS ColD
      ,CAST(REPLACE(CAST(CURRENT_TIMESTAMP AS DATE), '-', '') AS VARCHAR) AS ColE -- file transmission date
...
SELECT 'PD3' AS ColA
      ...
      ,CAST(REPLACE(CAST(co.DOB AS DATE),'-','') AS VARCHAR) AS ColI -- date of birth
      ,co.student_number AS ColJ -- student ID
      ,'Regular Diploma' AS ColK -- diploma type
      ,CAST(REPLACE(CAST(co.exitdate AS DATE),'-','') AS VARCHAR) AS ColL -- HS graduation date
