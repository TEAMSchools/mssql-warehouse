...
CREATE OR ALTER VIEW
  tableau.parcc_cma_correlation AS
SELECT
...
FROM
  ...
  JOIN gabby.parcc.summative_record_file_clean parcc ON co.student_number = parcc.local_student_identifier
  AND co.academic_year = parcc.academic_year
  AND asr.subject_area = CASE
    WHEN parcc.subject = 'English Language Arts/Literacy' THEN 'Text Study'
    ELSE 'Mathematics'
  END
WHERE
  co.rn_year = 1
