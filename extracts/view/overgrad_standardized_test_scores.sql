...
SELECT
  ...
  CONCAT(
    CASE
      ...
      WHEN stl.test_type = 'SAT'
      AND stl.score_type LIKE '%pre_2016%' THEN 'Old SAT'
      WHEN stl.test_type = 'SAT'
      AND stl.score_type NOT LIKE '%essay%' THEN 'New SAT'
      ELSE stl.test_type
    END,
    ' ' + CASE
...
FROM
  ...
  JOIN gabby.alumni.enrollment_identifiers ei ON stl.contact_c = ei.student_c
