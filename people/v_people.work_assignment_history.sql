...
WITH work_assignment_union AS (
  ...
  INNER JOIN gabby.people.employee_numbers sr
   ...
   AND sr.is_active = 1
  WHERE '2021-01-01' BETWEEN CAST(wah.position_effective_date AS DATE) 
                         AND COALESCE(CAST(wah.position_effective_end_date AS DATE), CURRENT_TIMESTAMP)
...
SELECT employee_number
      ...
      ,COALESCE(
        ...
        ,DATEADD(DAY, -1, LEAD(position_effective_date, 1) OVER(PARTITION BY position_id ORDER BY position_effective_date))
        ...
        ,DATEFROMPARTS(
           CASE
            WHEN DATEPART(YEAR, position_effective_date) > gabby.utilities.GLOBAL_ACADEMIC_YEAR()
             AND DATEPART(MONTH, position_effective_date) >= 7
                 THEN DATEPART(YEAR, position_effective_date) + 1
