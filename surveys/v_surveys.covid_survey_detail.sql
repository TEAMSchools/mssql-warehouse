...
WITH identifiers AS (
  SELECT survey_response_id
        ,covid_living 
        ,gender
  FROM (SELECT d.survey_response_id
    ...
    pivot(MAX(answer) for question_shortname in (covid_living, gender)
         ) p
  )

,ethnicity AS (
  SELECT rd.*
        ,CASE WHEN JSON_QUERY(rd.options, '$."11194"') IS NOT NULL THEN 1 ELSE 0 END AS is_asian
        ,CASE WHEN JSON_QUERY(rd.options, '$."11195"') IS NOT NULL THEN 1 ELSE 0 END AS is_black
        ,CASE WHEN JSON_QUERY(rd.options, '$."11196"') IS NOT NULL THEN 1 ELSE 0 END AS is_caucasian
        ,CASE WHEN JSON_QUERY(rd.options, '$."11197"') IS NOT NULL THEN 1 ELSE 0 END AS is_hispanic
        ,CASE WHEN JSON_QUERY(rd.options, '$."11198"') IS NOT NULL THEN 1 ELSE 0 END AS is_nativeamerican
...
SELECT d.survey_id
      ...
      ,e.is_other
      ,CASE WHEN (e.is_asian + e.is_black + e.is_caucasian + e.is_hispanic + e.is_nativeamerican + e.is_pacificislander + e.is_other) > 1 
            THEN 1 
            ELSE 0 
