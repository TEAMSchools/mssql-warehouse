...
CREATE OR ALTER VIEW dayforce.employee_benefits_current AS
...
FROM 
    (
     SELECT sub.employee_number
           ...
           ,ROW_NUMBER() OVER(
              PARTITION BY sub.employee_number, sub.ben_plan_category
                ORDER BY sub.ben_plan_option_effective_start) AS rn_curr
     FROM
         (
          SELECT employee_number                       
                ...
                ,CASE
                  WHEN ben_plan_name = 'Aetna HDHP' OR ben_plan_option_name = 'Waive Medical' THEN 'Medical'
...
PIVOT (
  ...
  FOR ben_plan_category IN ([Dental], [Verizon Wireless], [Basic Life Insurance], [Basic AD&D Insurance], [FSA-Dep]
                           ,[Life & AD&D supplemental], [Imputed Income - Life Insurance], [403b], [HSA]
                           ,[Employee Assistance Program], [Voluntary Accident Coverage], [Vision], [Medical], [FSA-HC]
