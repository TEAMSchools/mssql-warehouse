...
SELECT
  ...
  att.att_date,
  CAST(
    att.attendance_codeid AS INT
  ) AS attendance_codeid,
  CAST(
    att.att_mode_code AS VARCHAR(25)
  ) AS att_mode_code,
  CAST(
    att.calendar_dayid AS INT
  ) AS calendar_dayid,
  CAST(
    att.att_interval AS INT
  ) AS att_interval,
  CAST(att.ccid AS INT) AS ccid,
  CAST(att.periodid AS INT) AS periodid,
  CAST(att.programid AS INT) AS programid,
  CAST(
    att.total_minutes AS INT
  ) AS total_minutes,
  CAST(
    CASE
