CREATE OR ALTER VIEW
  nwea.assessment_result_identifiers AS
WITH
  results_clean AS (
    SELECT
    ...
    SELECT
      ...
      ROW_NUMBER() OVER (
        PARTITION BY
          ...
          sub.measurement_scale
        ORDER BY
          sub.growth_measure_yn DESC,
          sub.test_start_date DESC,
          sub.test_standard_error ASC
      ) AS rn_term_subj
    FROM
      results_clean AS sub
      INNER JOIN powerschool.cohort_static AS co ON (
        sub.student_id = co.student_number
