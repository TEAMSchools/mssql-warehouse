...
SELECT
  ...
  CONCAT(
    CASE
      ...
      WHEN (
        stl.test_type = 'SAT'
        AND stl.score_type LIKE '%pre_2016%'
      ) THEN 'Old SAT'
      WHEN (
        stl.test_type = 'SAT'
        AND stl.score_type NOT LIKE '%essay%'
      ) THEN 'New SAT'
      ELSE stl.test_type
    END,
    ' ' + (
      CASE
        ...
        WHEN (
          stl.test_subject = 'Studio Art: 2-D Design Portfolio'
        ) THEN 'Studio Art: 2-D Design'
        WHEN (
          stl.test_subject = 'Studio Art: Drawing Portfolio'
        ) THEN 'Studio Art: Drawing'
...
FROM
  ...
  INNER JOIN alumni.enrollment_identifiers AS ei ON (stl.contact_c = ei.student_c)
