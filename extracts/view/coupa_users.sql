...
WITH
  ...
  all_users AS (
    ...
    FROM
      ...
      INNER JOIN roles r ON cu.id = r.[user_id]
...
FROM
  (
    ...
    FROM
      all_users au
      INNER JOIN gabby.adsi.user_attributes_static ad ON au.employee_number = ad.employeenumber
      AND ISNUMERIC(ad.employeenumber) = 1
      LEFT JOIN gabby.coupa.school_name_lookup sn ON au.business_unit_code = sn.business_unit_code
      AND au.home_department = sn.home_department
      AND au.job_title = sn.job_title
      LEFT JOIN gabby.coupa.school_name_lookup sn2 ON au.business_unit_code = sn2.business_unit_code
      AND au.home_department = sn2.home_department
      AND sn2.job_title = 'Default'
      LEFT JOIN gabby.coupa.user_exceptions x ON au.employee_number = x.employee_number
      LEFT JOIN gabby.coupa.address_name_crosswalk anc ON au.[location] = anc.adp_location
      LEFT JOIN gabby.coupa.[address] a ON anc.coupa_address_name = a.[name]
      AND a.active = 1
  ) sub
  LEFT JOIN gabby.coupa.school_name_aliases sna ON sub.coupa_school_name = sna.physical_delivery_office_name
