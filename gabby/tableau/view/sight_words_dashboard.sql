...
SELECT
  ...
  0 AS is_replacement
FROM
  illuminate_codes.dna_scopes AS ds
  INNER JOIN illuminate_dna_repositories.repositories AS r ON (
  ...
  LEFT JOIN illuminate_dna_repositories.sight_words_data AS sw ON (
...
FROM
  ...
  INNER JOIN illuminate_dna_repositories.sight_words_data AS sw ON (
    co.student_number = sw.local_student_id
    AND r.repository_id = sw.repository_id
    AND f.[label] = sw.[label]
  )
