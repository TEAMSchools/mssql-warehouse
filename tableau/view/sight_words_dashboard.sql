...
FROM
  ...
  JOIN gabby.illuminate_dna_repositories.repository_grade_levels g ON r.repository_id = g.repository_id
  JOIN gabby.powerschool.cohort_identifiers_static co ON g.grade_level_id = (co.grade_level + 1)
...
FROM
  ...
  AND co.is_enrolled_recent = 1
  AND co.rn_year = 1
  JOIN gabby.illuminate_dna_repositories.sight_words_data sw ON co.student_number = sw.local_student_id
  AND r.repository_id = sw.repository_id
  AND f.[label] = sw.[label]
WHERE
  ds.code_translation = 'Sight Words Quiz'
