...
WITH
    clean_people AS (
        SELECT
            ...
            END AS is_hispanic,
            CASE
                WHEN sub.race_reporting IS NULL
                AND sub.ethnicity = 'Hispanic or Latino' THEN 'Hispanic or Latino'
...
,
        ...
        FROM
            (
                SELECT
                    ea.file_number AS df_employee_number,
...
,
                    ROW_NUMBER() OVER (
                        PARTITION BY
                            ea.associate_id
                        ORDER BY
                            CAST(e.position_start_date AS DATE) DESC
                    ) AS rn
                    /* use DF until fixed */
,
