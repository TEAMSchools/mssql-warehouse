...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN kippnewark.powerschool.students ps ON s.studentnumber = ps.student_number
    UNION ALL
    SELECT
      CAST(s.studentnumber AS INT) AS studentnumber,
      s.applicationid AS currentapplicationid,
    ...
    FROM
      ...
      INNER JOIN kippcamden.powerschool.students ps ON s.studentnumber = ps.student_number
  ) sub
