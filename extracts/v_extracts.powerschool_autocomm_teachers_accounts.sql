...
WITH users_union AS (
  ...
  INNER JOIN gabby.people.school_crosswalk sc
    ON scw.primary_site = sc.site_name
   AND sc._fivetran_deleted = 0
  INNER JOIN gabby.powerschool.users u
    ON scw.ps_teachernumber = u.teachernumber COLLATE Latin1_General_BIN
   ...
   AND CASE 
       ...
       END = u.[db_name]
  WHERE DATEDIFF(DAY, ISNULL(scw.termination_date, CAST(CURRENT_TIMESTAMP AS DATE)), CURRENT_TIMESTAMP) <= 14 /* import terminated staff up to a week after termination date */
    AND (scw.primary_on_site_department <> 'Data' OR scw.primary_on_site_department IS NULL)
...
,users_clean AS (
  /* existing users, DF schoolid matches PS homeschoolid */
  SELECT ps_teachernumber AS teachernumber
...
SELECT teachernumber
      ,first_name
      ,last_name
      ,CASE WHEN [status] = 1 THEN loginid END AS loginid
      ,CASE WHEN [status] = 1 THEN teacherloginid END AS teacherloginid
      ,email_addr
      ,CAST(COALESCE(homeschoolid, 0) AS INT) AS schoolid
      ,CAST(COALESCE(homeschoolid, 0) AS INT) AS homeschoolid
      ,[status]
      ,CASE WHEN [status] = 1 THEN 1 ELSE 0 END AS teacherldapenabled
      ,CASE WHEN [status] = 1 THEN 1 ELSE 0 END AS adminldapenabled
      ,CASE WHEN [status] = 1 THEN 1 ELSE 0 END AS ptaccess
      --,CASE 
