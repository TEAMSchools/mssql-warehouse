...
WITH
  long_data AS (
    ...
    FROM
      ...
      INNER JOIN gabby.illuminate_dna_assessments.agg_student_responses AS asr ON ais.assessment_id = asr.assessment_id -- trunk-ignore(sqlfluff/L016)
      ...
      INNER JOIN gabby.powerschool.cohort_identifiers_static AS co ON s.local_student_id = co.student_number -- trunk-ignore(sqlfluff/L016)
  ...
  scaled AS (
    ...
    FROM
      long_data
      LEFT JOIN gabby.act.scale_score_key AS ssk ON long_data.academic_year = ssk.academic_year -- trunk-ignore(sqlfluff/L016)
...
FROM
  ...
  LEFT JOIN gabby.illuminate_dna_assessments.agg_student_responses_standard AS std ON sub.assessment_id = std.assessment_id -- trunk-ignore(sqlfluff/L016)
  ...
  LEFT JOIN gabby.illuminate_standards.standards AS ps ON s.parent_standard_id = ps.standard_id -- trunk-ignore(sqlfluff/L016)
  LEFT JOIN gabby.illuminate_standards.standards AS ps2 ON ps.parent_standard_id = ps2.standard_id -- trunk-ignore(sqlfluff/L016)
