...
CREATE OR ALTER VIEW pm.teacher_goals_overall_scores AS
...
FROM
    (
     SELECT p.df_employee_number
           ...
           ,COALESCE((p.[Excellent Teaching Rubric] * 0.8) + (p.[Self & Others] * 0.2)
                    ,p.[Self & Others]
                    ,p.[Excellent Teaching Rubric]) AS overall_score
     FROM
         (
          SELECT tg.df_employee_number
                ,tg.preferred_name
                ,tg.primary_job
                ,tg.primary_site
                ,tg.academic_year
     ...
     PIVOT (
       MAX(metric_value_stored)
       FOR metric_label IN ([Excellent Teaching Rubric], [Self & Others])
      ) p
    ) sub
