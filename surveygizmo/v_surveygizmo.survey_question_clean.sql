...
SELECT id AS survey_question_id
      ...
      ,comment
      ,has_showhide_deps
      ,CAST(CASE WHEN shortname <> '' THEN shortname END AS NVARCHAR(128)) AS shortname
      ,CASE 
      ...
      ,CAST(JSON_VALUE(title, '$.English') AS NVARCHAR(2048)) AS title_english
      ,CAST(JSON_VALUE(properties, '$.url')  AS NVARCHAR(128)) AS [url]
      ,CAST(JSON_VALUE(properties, '$.orientation')  AS NVARCHAR(16)) AS orientation
      ...
      ,CAST(gabby.utilities.STRIP_HTML(JSON_VALUE(title, '$.English')) AS VARCHAR(2048)) AS title_clean
