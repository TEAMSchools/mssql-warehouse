CREATE OR ALTER VIEW
  tableau.marketing_facebook_post AS
SELECT
  ph.id,
  ...
  MAX(pm.post_reactions_like_total) + MAX(pm.post_reactions_love_total) + MAX(pm.post_reactions_wow_total) + MAX(pm.post_reactions_haha_total) + MAX(pm.post_reactions_sorry_total) + MAX(pm.post_reactions_anger_total) AS total_post_engagement,
  ph._fivetran_synced,
  ROW_NUMBER() OVER (
    PARTITION BY
      ph.id
    ORDER BY
      ph.updated_time DESC
  ) AS row_recent
FROM
  kipptaf.facebook_pages.post_history AS ph
  -- trunk-ignore(sqlfluff/LT05)
  INNER JOIN kipptaf.facebook_pages.lifetime_post_metrics_total AS pm ON (ph.id = pm.post_id)
GROUP BY
  ph.id,
  ph.updated_time,
  ph.created_time,
