  ...
  all_tables_columns_pivot AS (
    SELECT
      ...
      CASE
        ...
        WHEN sub.column_type_mismatch = 1
        AND sub.column_type_concat LIKE '%varchar%' THEN 'CAST([' + sub.column_name + '] AS NVARCHAR) AS [' + sub.column_name + ']'
        WHEN sub.column_type_mismatch = 1
        AND sub.column_type_concat LIKE '%float%' THEN 'CAST([' + sub.column_name + '] AS FLOAT) AS [' + sub.column_name + ']'
        WHEN sub.column_type_mismatch = 1
        AND sub.column_type_concat LIKE '%datetimeoffset%' THEN 'CAST([' + sub.column_name + '] AS DATETIMEOFFSET) AS [' + sub.column_name + ']'
        WHEN sub.column_type_mismatch = 1 THEN 'CAST([' + sub.column_name + '] AS INT) AS [' + sub.column_name + ']'
...
FROM
  (
    SELECT
      ...
      COUNT(
        CASE
          WHEN atc.kippcamden NOT LIKE '%NULL%' THEN atc.kippcamden
        END
      ) AS kippcamden_count,
      COUNT(
        CASE
          WHEN atc.kippmiami NOT LIKE '%NULL%' THEN atc.kippmiami
        END
      ) AS kippmiami_count
    FROM
