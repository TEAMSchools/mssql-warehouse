...
CREATE OR ALTER VIEW tntp.insight_index_scores AS

SELECT sub.administration_year
...
FROM
    (
     ...
     FROM gabby.tntp.insight_survey_detail
     WHERE survey_type = 'Teacher'
       AND is_ici = 1
...
GROUP BY sub.administration_year
        ,sub.administration_academic_year
        ,sub.administration_round
        ,sub.reporting_schoolid
        ,sub.school