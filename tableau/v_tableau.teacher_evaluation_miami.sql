...
WITH job_last_year AS (
  ...
  WHERE business_unit_code = 'KIPP_MIAMI'
    AND work_assignment_start_date <= DATEFROMPARTS(
          gabby.utilities.GLOBAL_ACADEMIC_YEAR(), 6, 30
        )
)
...
SELECT c.df_employee_number
      ...
      ,CASE
        ...
        WHEN s.overall_tier = 2 
         AND w.[Years Teaching - In any State] >= 2 
             THEN 'E'
        WHEN s.overall_tier = 2 
         AND w.[Years Teaching - In any State] <= 3 
             THEN 'F'
        WHEN s.overall_tier = 2 
         AND c.original_hire_date <= DATEFROMPARTS(
               gabby.utilities.GLOBAL_ACADEMIC_YEAR() - 2, 6, 30
             )
             THEN 'E' 
        WHEN s.overall_tier = 2 
         AND c.original_hire_date <= DATEFROMPARTS(
               gabby.utilities.GLOBAL_ACADEMIC_YEAR() - 3, 6, 30
             )
             THEN 'F' 
        WHEN s.overall_tier = 1 THEN 'G'
        WHEN c.primary_job NOT IN (
               'Teacher','Teacher in Residence','Learning Specialist','Assistant School Leader',
               'School Leader','Dean of Students'
             ) THEN 'Z' 
        ELSE 'I'
...
LEFT JOIN gabby.pm.teacher_goals_overall_scores_static s
