...
SELECT CASE WHEN MONTH(CONVERT(DATE,sur.date_submitted)) < 6
            THEN 'spring'
            ELSE 'fall'
            END AS term_name
       ,gabby.utilities.DATE_TO_SY(date_submitted) AS academic_year
       ,sur.please_confirm_your_email_address_please_use_your_kippnj_org_address_for_tracking_purposes_only_ AS email
...
FROM to_survey 
     LEFT OUTER JOIN adp.staff_roster roster
     ON to_survey.associate_id = roster.associate_id
...
to_long AS (
 ...
 SELECT term_name
        ...
        ,location
        ,question
        ,CASE WHEN response = 'Strongly Agree' THEN '5'
              ...
              ELSE response 
              END AS response
        ,CASE WHEN question IN ('tech_training'
                                ,'tech_oe')
              THEN 'oe'
              WHEN question = 'tech_submit_tickets'
              THEN 'ticket'
