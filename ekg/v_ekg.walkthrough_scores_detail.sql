...
WITH archive_unpivot AS (
  ...
  UNPIVOT(
    pct_of_classrooms_proficient
    FOR rubric_strand_field IN (culture_arrival_dismissal
                               ,culture_bathrooms
...
SELECT school
      ...
      ,ROW_NUMBER() OVER(
         PARTITION BY reporting_schoolid, academic_year, rubric_strand_field
           ORDER BY reporting_term DESC) AS rn_most_recent_yr
FROM
    (
     SELECT su.school
           ,su.academic_year
