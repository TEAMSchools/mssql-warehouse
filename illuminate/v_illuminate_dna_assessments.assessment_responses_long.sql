...
WITH asmts AS (
  ...
  FROM gabby.illuminate_dna_assessments.student_assessment_scaffold_current_static a
...
FROM asmts a  
INNER JOIN gabby.illuminate_dna_assessments.agg_student_responses_standard asrs
  ON a.student_assessment_id = asrs.student_assessment_id
 AND asrs.points_possible > 0  
INNER JOIN gabby.illuminate_dna_assessments.assessment_standards astd
  ON asrs.assessment_id = astd.assessment_id
 AND asrs.standard_id = astd.standard_id
INNER JOIN gabby.illuminate_standards.standards std
...
SELECT a.student_id
      ...
      ,NULL AS domain_description
FROM asmts a  
INNER JOIN gabby.illuminate_dna_assessments.agg_student_responses_group asrg
  ON a.student_assessment_id = asrg.student_assessment_id      
 AND asrg.points_possible > 0       
INNER JOIN gabby.illuminate_dna_assessments.assessments_reporting_groups arg
  ON asrg.assessment_id = arg.assessment_id
 AND asrg.reporting_group_id = arg.reporting_group_id
INNER JOIN gabby.illuminate_dna_assessments.reporting_groups rg
