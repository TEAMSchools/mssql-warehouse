...
ALTER VIEW asset_details AS

WITH current_trans AS (
  SELECT asset.asset_id
        ...
        ,ROW_NUMBER() OVER(
           PARTITION BY asset.asset_id
             ORDER BY ats.entry_date DESC) AS rn_entrydate
  ...
  JOIN [WaspTrackAsset].[dbo].[employees]
    ON ats.employee_id = employees.employee_id   
 )

SELECT asset.[asset_id] AS [Asset ID]
...
INNER JOIN [WaspTrackAsset].[dbo].[departments]
  ON asset.department_id = departments.department_id  
INNER JOIN [WaspTrackAsset].[dbo].[location]
  ON asset.location_id = location.location_id  
INNER JOIN [WaspTrackAsset].[dbo].[category]
  ON item.category_id = category.category_id
INNER JOIN [WaspTrackAsset].[dbo].[condition]
  ON asset.condition_id = condition.condition_id
INNER JOIN [WaspTrackAsset].[dbo].[sites]
  ON location.site_id = sites.site_id
LEFT JOIN current_trans ct
  ON asset.asset_id = ct.asset_id
