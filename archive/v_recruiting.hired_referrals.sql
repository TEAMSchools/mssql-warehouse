...
LEFT JOIN gabby.recruiting.job_application_c a 
  ON LEFT(c.contact_c, LEN(c.contact_c) - 3) = a.contact_id_c
 AND a.stage_c = 'Hired' 
LEFT JOIN gabby.recruiting.profile_application_c pa
  ON a.profile_application_c = pa.id
LEFT JOIN gabby.recruiting.job_position_c p
  ON a.job_position_c = p.id
LEFT JOIN gabby.dayforce.staff_roster df
  ON p.name = df.salesforce_id
WHERE c.regional_source_c = 'Referral' 
  AND (c.cultivation_stage_c = 'Hired' OR a.stage_c = 'Hired')