...
WITH
  ...
  ethnicity AS (
    SELECT
      survey_response_id,
      CASE
        WHEN JSON_QUERY(options, '$."11194"') IS NOT NULL THEN 1
        ELSE 0
      END AS is_asian,
      CASE
        WHEN JSON_QUERY(options, '$."11195"') IS NOT NULL THEN 1
        ELSE 0
      END AS is_black,
      CASE
        WHEN JSON_QUERY(options, '$."11196"') IS NOT NULL THEN 1
        ELSE 0
      END AS is_caucasian,
      CASE
        WHEN JSON_QUERY(options, '$."11197"') IS NOT NULL THEN 1
        ELSE 0
      END AS is_hispanic,
      CASE
        WHEN JSON_QUERY(options, '$."11198"') IS NOT NULL THEN 1
        ELSE 0
      END AS is_nativeamerican,
      CASE
        WHEN JSON_QUERY(options, '$."11199"') IS NOT NULL THEN 1
        ELSE 0
      END AS is_pacificislander,
      CASE
        WHEN JSON_QUERY(options, '$."11200"') IS NOT NULL THEN 1
        ELSE 0
      END AS is_na,
      CASE
        WHEN JSON_QUERY(options, '$."11202"') IS NOT NULL THEN 1
        ELSE 0
      END AS is_other
    FROM
...
FROM
  ...
  LEFT JOIN identifiers AS i ON (
    d.survey_response_id = i.survey_response_id
  )
  LEFT JOIN ethnicity AS e ON (
    d.survey_response_id = e.survey_response_id
  )
WHERE
  d.survey_id = 5560557
