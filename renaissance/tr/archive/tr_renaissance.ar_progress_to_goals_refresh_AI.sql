USE gabby GO IF OBJECT_ID('renaissance.TR_ar_progress_to_goals_refresh_AI', 'TR') IS NOT NULL DROP
TRIGGER renaissance.TR_ar_progress_to_goals_refresh_AI;

GO CREATE
TRIGGER renaissance.TR_ar_progress_to_goals_refresh_AI ON renaissance.fivetran_audit AFTER
INSERT
...
SET
  @schema_name = 'renaissance'
SET
  @view_name = 'ar_progress_to_goals' BEGIN WAITFOR DELAY '00:01:00';

/* manually added minute wait */
BEGIN TRY
...
WHERE
  ...
  AND table_name IN ('ar_studentpractice', 'rl_user');
...
IF @update_status = 0 BEGIN PRINT ('All tables referenced by view have not yet been updated today');
