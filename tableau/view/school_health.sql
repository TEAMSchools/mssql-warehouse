...
UNION ALL
SELECT
  LOWER(grade_band) + '_gpa' AS DOMAIN,
  sub.academic_year,
  sub.schoolid,
  sub.grade_band,
...
UNION ALL
SELECT
  'i-ready_typical_' + sub.iready_subject AS DOMAIN,
  sub.academic_year,
  sub.schoolid,
  sub.grade_band,
...
UNION ALL
SELECT
  'i-ready_stretch_math' AS DOMAIN,
  sub.academic_year,
  sub.schoolid,
  sub.grade_band,
...
UNION ALL
SELECT
  'i-ready_on_grade_' + sub.iready_subject AS DOMAIN,
  sub.academic_year,
  sub.schoolid,
  sub.grade_band,
...
WHERE
  ...
  AND mem.yearid >= (
    (gabby.utilities.GLOBAL_ACADEMIC_YEAR () - 3) - 1990
  )
GROUP BY
  co.academic_year,
  co.schoolid
...
UNION ALL
SELECT
  'student_attrition' AS DOMAIN,
  sub.academic_year,
  sub.schoolid,
  sub.grade_band,
  ROUND(
    1 - ROUND(AVG(CAST(sub.is_enrolled_next AS FLOAT)), 2),
    2
  ) AS pct_met_goal,
  ROUND(
    1 - ROUND(
...
FROM
  (
    SELECT
      ...
      CASE
        WHEN ROUND(AVG(CAST(cm.answer_value AS FLOAT)), 2) >= 4 THEN 1
        ELSE 0
      END AS avg_response,
...
SELECT
  ...
  ROUND(
    AVG(
      CASE
        WHEN sub.iep_status = 'SPED' THEN CAST(sub.response_value AS FLOAT)
        ELSE NULL
      END
    ),
    2
  ...
  ROUND(
    AVG(
      CASE
        WHEN sub.gender = 'F' THEN CAST(sub.response_value AS FLOAT)
        ELSE NULL
      END
    ),
    2
  ...
  ROUND(
    AVG(
      CASE
        WHEN sub.gender = 'M' THEN CAST(sub.response_value AS FLOAT)
        ELSE NULL
      END
    ),
    2
  ...
  (
    ...
    AND co.rn_year = 1 UNPIVOT (
      response FOR question_text IN (
        ...
        _2_what_best_describes_the_interactions_
        --BETWEEN_staff_and_students_
,
...
UNION ALL
SELECT
  'state_asmt' + '_' + sub.[subject] AS DOMAIN,
  sub.academic_year,
  sub.schoolid,
