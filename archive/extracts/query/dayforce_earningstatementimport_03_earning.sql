CREATE OR ALTER VIEW
  extracts.dayforce_earningstatementimport_03_earning AS
  /* 1 - Regular Earnings */
SELECT
...
SELECT
  ...
  SUM(
    CASE
      WHEN void_check_indicator = 'Y' THEN NULL
      ELSE additional_hours
    END
  ) OVER (
    PARTITION BY
      associate_id,
      YEAR(pay_date),
      additional_earnings_code_pay_statements
    ORDER BY
      pay_date
  ) AS HoursYTD,
  SUM(
