...
CREATE OR ALTER VIEW extracts.njdoe_bgcheck_audit AS
    ...
    (
     SELECT employee_number
           ...
           ,CASE 
             ...
             WHEN business_unit_code = 'KIPP_TAF' AND is_approved = 1 THEN 1
             ELSE 0
            END AS is_approved_district
           ,ROW_NUMBER() OVER(
              PARTITION BY employee_number
              ORDER BY approvaldate DESC, approved_business_unit_code DESC
             ) AS rn_emp
           --,business_unit_code
           --,approvaldate
           --,approved_districtcode
     ...
     FROM
         (
          ...
          LEFT JOIN gabby.njdoe.background_check_approval_history bg
            ON sr.employee_number = bg.employee_number
          WHERE sr.position_status <> 'Terminated'
