...
CREATE OR ALTER VIEW extracts.gsheets_dlm_roster AS
...
WITH roster AS (
  ...
  JOIN gabby.powerschool.s_nj_stu_x nj
    ON co.students_dcid = nj.studentsdcid
   AND co.[db_name] = nj.[db_name]
   AND (nj.state_assessment_name IN (3, 4) OR nj.math_state_assessment_name = 3)
  WHERE co.academic_year = gabby.utilities.GLOBAL_ACADEMIC_YEAR()
    AND co.rn_year = 1
    AND (co.grade_level BETWEEN 3 AND 8 OR co.grade_level = 11)
 )
...
WHERE r.state_assessment_name IS NOT NULL
...
WHERE r.math_state_assessment_name IS NOT NULL
