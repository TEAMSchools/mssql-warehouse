...
IF OBJECT_ID ('renaissance.TR_ar_progress_to_goals_refresh_AI','TR') IS NOT NULL
   DROP TRIGGER renaissance.TR_ar_progress_to_goals_refresh_AI;
GO

CREATE TRIGGER renaissance.TR_ar_progress_to_goals_refresh_AI ON renaissance.fivetran_audit
  AFTER INSERT
AS
...
SET @view_name = 'ar_progress_to_goals'
...
BEGIN
  BEGIN TRY
    ...
    WHERE usedbyschemaname = @schema_name
      ...
      AND table_name IN ('ar_studentpractice','rl_user'); /* manual exclude */
    ...
    IF @update_status = 0
      BEGIN
        PRINT('All tables referenced by view have not yet been updated today');
        RETURN    
      END
