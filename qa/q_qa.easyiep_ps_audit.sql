WITH ps AS (
  ...
  WHERE s.entrydate >= '2021-07-01'
    AND s.grade_level <> 99
    AND s.[db_name] <> 'kippmiami'
 )

,iep AS (
  SELECT iep.student_number
        ,iep.state_studentnumber
        ,iep.lastfirst
        ,iep.spedlep
        ,iep.special_education
        ,iep.special_education_code
  FROM gabby.easyiep.njsmart_powerschool_clean iep
  WHERE iep.academic_year = 2021
 )
...
FROM
    (
     ...
     FROM ps
     FULL JOIN iep
       ON ps.student_number = iep.student_number
    ) sub
