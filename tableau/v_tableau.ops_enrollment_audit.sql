...
WITH residency_verification AS (
  ...
  FROM gabby.powerschool.u_def_ext_students x
  JOIN gabby.powerschool.students s
    ON x.studentsdcid = s.dcid
   AND x.[db_name] = s.[db_name]
   AND s.enroll_status = 0
  LEFT JOIN gabby.enrollment.residency_verification rv
    ON s.student_number = rv.id
  ...
  FROM
      (
       SELECT s.student_number
             ...
             ,CASE
               WHEN (s.enroll_status = -1 OR COALESCE(co.year_in_network, 1) = 1) THEN ISNULL(rv.residency_proof_2, 'Missing') 
       ...
       LEFT JOIN residency_verification rv
         ON s.student_number = rv.student_number
        AND rv.rn = 1
