  ...
  FROM
      (
       SELECT co.student_number
             ...
             ,CASE WHEN arsp.rn_quiz = 1 THEN arsp.i_quiz_number END AS is_total
...
SELECT pr.student_number
      ...
      ,CASE
        ...
        ELSE CASE
              WHEN (pr.words IS NULL OR goals.words_goal IS NULL) THEN NULL
              WHEN pr.words >= ROUND((pr.n_days_elapsed / pr.n_days_term) * goals.words_goal, 0) THEN NULL
              ELSE ROUND((pr.n_days_elapsed / pr.n_days_term) * goals.words_goal, 0) - pr.words
             END
       END AS words_needed
