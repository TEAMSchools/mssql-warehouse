...
FROM
  (
    ...
    FROM
      gabby.surveygizmo.survey_response_identifiers_static sri
      INNER JOIN gabby.surveygizmo.survey_response_data sd ON sri.survey_id = sd.survey_id
      AND sri.survey_response_id = sd.survey_response_id
      INNER JOIN gabby.surveygizmo.survey_question_clean_static sq ON sd.survey_id = sq.survey_id
      AND sd.question_id = sq.survey_question_id
      AND sq.base_type = 'Question'
      LEFT JOIN gabby.surveygizmo.survey_question_options_static qo ON sd.survey_id = qo.survey_id
