...
FROM
  [WINSQL06\HAN].[Franklin].[dbo].[VIEW_STUDENT_DATA] AS s
  LEFT JOIN [WINSQL06\HAN].[Newton].[dbo].[STUDENT_GUID_LINK] AS g ON (
    s.[StudentGuid] = g.[StudentGUID]
  )
  LEFT JOIN [WINSQL06\HAN].[Newton].[dbo].[CUSTOMER] AS c ON (
    g.[CustomerID] = c.[CUSTOMER_RECID]
  )
WHERE
...
SELECT
  ...
  c.[ReimbursableOnlyBalance] + c.[UnallocatedBalance] AS total_balance
FROM
  [WINSQL06\YODA].[Franklin].[dbo].[VIEW_STUDENT_DATA] AS s
  LEFT JOIN [WINSQL06\YODA].[Newton].[dbo].[STUDENT_GUID_LINK] AS g ON (
    s.[StudentGuid] = g.[StudentGUID]
  )
  LEFT JOIN [WINSQL06\YODA].[Newton].[dbo].[CUSTOMER] AS c ON (
    g.[CustomerID] = c.[CUSTOMER_RECID]
  )
WHERE
  ISNUMERIC(s.[StudentNumber]) = 1;
