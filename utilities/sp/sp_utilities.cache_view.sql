...
BEGIN
  ...
  IF OBJECT_ID(@source_view, 'V') IS NULL
    BEGIN
      RAISERROR('View does not exist', 0, 1) WITH NOWAIT;
      RETURN;
    END;
  ...
  IF OBJECT_ID(@destination_table_name) IS NULL
    BEGIN
      ...
      BEGIN TRY
        BEGIN TRANSACTION
          RAISERROR(@sql_create, 0, 1) WITH NOWAIT;
          EXEC (@sql_create);
        COMMIT
      END TRY
      BEGIN CATCH
  ...
  ELSE
    BEGIN
      ...
      BEGIN TRY
        BEGIN TRANSACTION
          RAISERROR(@sql_drop1, 0, 1) WITH NOWAIT;
          EXEC (@sql_drop1);
        COMMIT
      END TRY
      BEGIN CATCH
      ...
      BEGIN TRY
        BEGIN TRANSACTION
          RAISERROR(@sql_selectinto, 0, 1) WITH NOWAIT;
          EXEC (@sql_selectinto);
        COMMIT
      END TRY
      BEGIN CATCH
      ...
      IF @@ROWCOUNT > 0
        BEGIN
          BEGIN TRY
            BEGIN TRANSACTION
              RAISERROR(@sql_truncateinsert, 0, 1) WITH NOWAIT;
      ...
      BEGIN TRY
        BEGIN TRANSACTION
          RAISERROR(@sql_drop2, 0, 1) WITH NOWAIT;
          EXEC (@sql_drop2);
        COMMIT
      END TRY
      BEGIN CATCH
