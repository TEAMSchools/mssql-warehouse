...
WITH clean_data AS (
  ...
  SELECT CAST(student_number AS INT) AS student_number
        ,date_administered
        ...
        ,reading_rate_wpm
        ,CASE WHEN instructional_level_tested <> '' THEN CAST(instructional_level_tested AS VARCHAR(5)) END AS instructional_level_tested
        ,CASE WHEN rate_proficiency <> '' THEN CAST(rate_proficiency AS VARCHAR(25)) END AS rate_proficiency
        ,CASE WHEN key_lever <> '' THEN CAST(key_lever AS VARCHAR(25)) END AS key_lever
        ,CASE WHEN fiction_nonfiction <> '' THEN CAST(fiction_nonfiction AS VARCHAR(5)) END AS fiction_nonfiction
        ,NULL AS test_administered_by
