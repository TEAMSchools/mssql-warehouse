...
FROM
  ...
  INNER JOIN gabby.powerschool.final_grades_wide_static fg ON co.studentid = fg.studentid
  AND co.yearid = fg.yearid
  AND co.[db_name] = fg.[db_name]
  AND fg.reporting_term <> 'CUR'
  INNER JOIN gabby.powerschool.sections_identifiers sec ON fg.sectionid = sec.sectionid
  ...
  LEFT JOIN gabby.powerschool.category_grades_wide_static cat ON co.studentid = cat.studentid
  AND fg.course_number = cat.course_number
  AND fg.reporting_term = cat.reporting_term
  AND fg.[db_name] = cat.[db_name]
  LEFT JOIN gabby.powerschool.category_grades_wide_static kctz ON co.studentid = kctz.studentid
  ...
  LEFT JOIN gabby.powerschool.pgfinalgrades comm ON fg.studentid = comm.studentid
