...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN utilities.row_generator AS n ON (n.n <= 5)
  ) AS sub
  LEFT JOIN alumni.enrollment_c AS e ON (
  ...
  LEFT JOIN alumni.account AS a ON (e.school_c = a.id)
  LEFT JOIN alumni.enrollment_identifiers AS eis ON (
