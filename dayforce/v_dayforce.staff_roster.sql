...
WITH clean_people AS (
  ...
  FROM
      (
       SELECT CAST(e.df_employee_number AS INT) AS df_employee_number
             ,CASE WHEN e.adp_associate_id_clean <> '' THEN CAST(e.adp_associate_id_clean AS VARCHAR(25)) END AS adp_associate_id
             ,CASE WHEN e.salesforce_id <> '' THEN CAST(e.salesforce_id AS VARCHAR(25)) END AS salesforce_id
             ,CASE WHEN e.first_name <> '' THEN CAST(e.first_name AS VARCHAR(25)) END AS first_name
             ...
             ,CASE WHEN e.primary_on_site_department_clean <> '' THEN CAST(e.primary_on_site_department_clean AS VARCHAR(125)) END AS primary_on_site_department
             ,CASE WHEN e.legal_entity_name_clean <> '' THEN CAST(e.legal_entity_name_clean AS VARCHAR(125)) END AS legal_entity_name
             ,CASE WHEN e.job_family <> '' THEN CAST(e.job_family AS VARCHAR(25)) END AS job_family
             ,CASE WHEN e.payclass <> '' THEN CAST(e.payclass AS VARCHAR(5)) END AS payclass
             ,CASE WHEN e.paytype <> '' THEN CAST(e.paytype AS VARCHAR(25)) END AS paytype
             ,CASE WHEN e.jobs_and_positions_flsa_status <> '' THEN CAST(e.jobs_and_positions_flsa_status AS VARCHAR(25)) END AS flsa_status
             ,CASE WHEN e.grades_taught <> '' THEN CAST(e.grades_taught AS VARCHAR(125)) END AS grades_taught
