...
SELECT
    ...
    CASE
        WHEN co.enroll_status = 0 THEN COALESCE(c.email, co.student_web_id + '@teamstudents.org')
        ELSE c.email
...
FROM
    ...
    LEFT JOIN gabby.powerschool.student_contacts_wide_static AS sc ON CAST(sc.student_number AS nvarchar) = CAST(c.school_specific_id_c AS nvarchar)
    LEFT JOIN gabby.powerschool.cohort_identifiers_static AS co ON CAST(co.student_number AS nvarchar) = CAST(c.school_specific_id_c AS nvarchar)
