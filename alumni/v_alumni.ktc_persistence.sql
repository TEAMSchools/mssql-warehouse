...
CREATE OR ALTER VIEW alumni.ktc_persistence AS
...
LEFT JOIN gabby.alumni.enrollment_c e
  ...
  AND DATEFROMPARTS(sub.academic_year, 10, 31) BETWEEN e.start_date_c 
                                                   AND COALESCE(e.actual_end_date_c, DATEFROMPARTS((gabby.utilities.GLOBAL_ACADEMIC_YEAR() + 1), 6, 30))
  AND e.is_deleted = 0
...
LEFT JOIN gabby.alumni.enrollment_identifiers ei
  ON sub.sf_contact_id = ei.student_c
