...
WITH salary_union AS (
  ...
  INNER JOIN gabby.people.employee_numbers sr
   ...
   AND sr.is_active = 1
  WHERE (CAST(sh.regular_pay_effective_date AS DATE) < CAST(sh.regular_pay_effective_end_date AS DATE) OR sh.regular_pay_effective_end_date IS NULL)
...
SELECT employee_number
      ...
      ,COALESCE(
        ...
        ,DATEADD(DAY, -1, LEAD(regular_pay_effective_date, 1) OVER(PARTITION BY position_id ORDER BY regular_pay_effective_date))
        ...
        ,DATEFROMPARTS(
           CASE
            WHEN DATEPART(YEAR, regular_pay_effective_date) > gabby.utilities.GLOBAL_ACADEMIC_YEAR()
             AND DATEPART(MONTH, regular_pay_effective_date) >= 7
