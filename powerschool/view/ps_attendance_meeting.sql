...
FROM
  ...
  INNER JOIN powerschool.cc ON att.ccid = cc.id
  AND att.studentid = cc.studentid
  INNER JOIN powerschool.sections s ON ABS(cc.sectionid) = s.id
  ...
  INNER JOIN powerschool.period per ON att.periodid = per.id
WHERE
