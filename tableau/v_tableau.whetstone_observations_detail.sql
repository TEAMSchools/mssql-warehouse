...
WITH boxes AS (
  SELECT tb.observation_id AS observation_id
        ,tb.score_measurement_id AS score_measurement_id
        ,tb.text_box_label AS label
        ,tb.text_box_text AS value
        ,'textbox' AS type 
  FROM gabby.whetstone.observations_scores_text_boxes tb


  UNION

  SELECT cc.observation_id AS observation_id
        ,cc.score_measurement_id AS score_measurement_id
        ,cc.checkbox_label AS label
        ,CONVERT(NVARCHAR, cc.checkbox_value) AS value
        ,'checkbox' AS type
  FROM whetstone.observations_scores_checkboxes cc

   )

SELECT sub.*
      ,COALESCE(CASE WHEN SUM(CASE WHEN b.type = 'checkbox' THEN CONVERT(INT, b.value) ELSE NULL END) OVER( PARTITION BY sub.observation_id, b.score_measurement_id) > 0 THEN CONVERT(FLOAT,b.value) ELSE NULL END
      ...
      ,CASE 
        WHEN b.label = 'textbox' THEN wm.[name]
        WHEN b.label = 'checkbox' THEN wm.[name] + ' - ' + b.label
        ELSE wm.[name]
       END AS measurement_label


      ,CASE 
        WHEN SUM(CASE WHEN b.type = 'checkbox' THEN CONVERT(INT, b.value) ELSE NULL END) OVER( PARTITION BY sub.observation_id, b.score_measurement_id) > 0 THEN 1 
        ELSE 0 
 ...
 FROM (
       SELECT sr.df_employee_number
             ...
             ,ROW_NUMBER()  OVER( PARTITION BY sr.df_employee_number, wo.rubric_name ORDER BY wo.observed_at DESC) AS rn_observation
...
LEFT JOIN boxes b
  ON wos.score_measurement_id = b.score_measurement_id
 AND sub.observation_id = b.observation_id