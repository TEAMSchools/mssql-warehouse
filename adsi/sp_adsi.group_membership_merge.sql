...
CREATE OR ALTER PROCEDURE adsi.group_membership_merge AS 
...
BEGIN
  ...
  IF OBJECT_ID(N'tempdb..#ad_group_membership') IS NOT NULL
    ...
    CREATE TABLE [#ad_group_membership] (
  ...
  DECLARE group_cursor CURSOR FOR
    SELECT REPLACE(ADsPath, 'LDAP://', '') AS ADsPath, cn
    FROM OPENQUERY(ADSI,'
      SELECT ADsPath, cn
    ...
    OPEN group_cursor;
      WHILE 1 = 1
        BEGIN
          FETCH NEXT FROM group_cursor INTO @group_adspath, @group_cn;
          IF @@FETCH_STATUS <> 0
            BEGIN
              BREAK;
            END
  ...
  IF OBJECT_ID(N'gabby.adsi.group_membership') IS NULL
      BEGIN
        SELECT *
    ...
    ELSE
      BEGIN
        ...
        MERGE gabby.adsi.group_membership AS TARGET
          USING [#ad_group_membership] AS SOURCE
             ON TARGET.group_adspath = SOURCE.group_adspath
            AND TARGET.object_guid = SOURCE.object_guid
          WHEN MATCHED THEN
            UPDATE
              SET TARGET.employee_number = SOURCE.employee_number
