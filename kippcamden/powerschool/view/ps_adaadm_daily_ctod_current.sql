...
WITH
  terms_attendance_code AS (
    ...
    FROM
      ...
      LEFT JOIN powerschool.attendance_code AS ac ON (
        t.schoolid = ac.schoolid
...
SELECT
  ...
  CASE
    ...
    ELSE CAST(
      aci_potential.attendance_value AS INT
    )
...
FROM
  ...
  LEFT JOIN powerschool.ps_attendance_daily_current_static AS ada_0 ON (
    mv.studentid = ada_0.studentid
    AND mv.calendardate = ada_0.att_date
    AND ada_0.count_for_ada = 0
  )
  LEFT JOIN powerschool.ps_attendance_daily_current_static AS ada_1 ON (
    mv.studentid = ada_1.studentid
    AND mv.calendardate = ada_1.att_date
    AND ada_1.count_for_ada = 1
  )
  LEFT JOIN powerschool.ps_attendance_daily_current_static AS adm_0 ON (
    mv.studentid = adm_0.studentid
    AND mv.calendardate = adm_0.att_date
    AND adm_0.count_for_adm = 0
  )
  LEFT JOIN aci AS aci_real ON (
    mv.fteid = aci_real.fteid
  ...
  LEFT JOIN aci AS aci_potential ON (
    mv.fteid = aci_potential.fteid
