...
,final_data AS (
  SELECT pas.position_id
        ...
        ,LAG(pas.business_unit_paydate, 1, pas.business_unit_paydate) OVER(
           PARTITION BY pas.fiscal_year, pas.code, pas.employee_number
             ORDER BY pas.payroll_date) AS business_unit_prev_paydate
        ,LAG(pas.location_paydate, 1, pas.location_paydate) OVER(
           PARTITION BY pas.fiscal_year, pas.code, pas.employee_number
             ORDER BY pas.payroll_date) AS location_prev_paydate
        ,LAG(pas.department_paydate, 1, pas.department_paydate) OVER(
           PARTITION BY pas.fiscal_year, pas.code, pas.employee_number
             ORDER BY pas.payroll_date) AS department_prev_paydate
        ,LAG(pas.job_title_paydate, 1, pas.job_title_paydate) OVER(
           PARTITION BY pas.fiscal_year, pas.code, pas.employee_number
             ORDER BY pas.payroll_date) AS job_title_prev_paydate
        ,LAG(pas.salary_paydate, 1, pas.salary_paydate) OVER(
           PARTITION BY pas.fiscal_year, pas.code, pas.employee_number
             ORDER BY pas.payroll_date) AS salary_prev_paydate
        ,LAG(pas.status_paydate, 1, pas.status_paydate) OVER(
           PARTITION BY pas.fiscal_year, pas.code, pas.employee_number
             ORDER BY pas.payroll_date) AS status_prev_paydate
  FROM pas
 )
