...
IF OBJECT_ID ('nwea.TR_assessment_result_identifiers_refresh_AI','TR') IS NOT NULL
   DROP TRIGGER nwea.TR_assessment_result_identifiers_refresh_AI;
GO

CREATE TRIGGER nwea.TR_assessment_result_identifiers_refresh_AI ON nwea.fivetran_audit
  AFTER INSERT
AS
...
SET @schema_name = 'nwea'
SET @view_name = 'assessment_result_identifiers'
...
BEGIN
  BEGIN TRY
    ...
    WHERE usedbyschemaname = @schema_name
      ...
      AND table_name NOT IN ('percentile_norms'); /* manual exclude */
    ...
    INTO #updated_tables
    FROM nwea.fivetran_audit WITH(NOLOCK)
