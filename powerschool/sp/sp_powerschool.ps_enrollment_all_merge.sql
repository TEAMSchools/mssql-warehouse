...
ALTER PROCEDURE powerschool.ps_enrollment_all_merge AS
...
DECLARE @email_subject NVARCHAR(MAX)
  ...
  BEGIN TRY
    IF OBJECT_ID(N'#ps_enrollment_all_temp') IS NOT NULL
    BEGIN
    ...
    FROM powerschool.ps_enrollment_all; 

    MERGE gabby.powerschool.ps_enrollment_all_static AS TARGET
      USING #ps_enrollment_all_temp AS SOURCE
         ON TARGET.studentid = SOURCE.studentid
        AND TARGET.schoolid = SOURCE.schoolid
        AND TARGET.entrydate = SOURCE.entrydate
      WHEN MATCHED THEN
       UPDATE
        SET TARGET.entrycode = SOURCE.entrycode
  ...
  BEGIN CATCH
    ...
    SET @email_subject = 'ps_enrollment_all static refresh failed'
    SET @email_body = 'The refresh procedure for ps_enrollment_all failed.' + CHAR(10) + ERROR_MESSAGE();
