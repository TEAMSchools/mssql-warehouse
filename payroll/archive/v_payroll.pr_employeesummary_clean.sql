...
,scaffold AS (
  ...
  JOIN dates d
    ON d.pay_date BETWEEN sr.position_start_date AND DATEADD(DAY, 16, COALESCE(sr.termination_date, CAST(CURRENT_TIMESTAMP AS DATE)))
 )
...
FROM
    (
     ...
     FROM scaffold s
     LEFT JOIN gabby.adp.pr_employeesummary pr
