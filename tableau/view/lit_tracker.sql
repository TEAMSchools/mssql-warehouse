...
WITH lit_round AS (
  SELECT co.student_number
        ...
        ,achv.lvl_num 
          - LAG(achv.lvl_num, 1) OVER (
              PARTITION BY achv.academic_year, achv.student_number 
              ORDER BY achv.[start_date] ASC
...
SELECT lr.student_number
      ...
      ,lr.words_goal
      ,ROW_NUMBER() OVER(
         PARTITION BY lr.student_number, lr.academic_year, lr.lit_term, lr.ar_term, lr.achv_unique_id
         ORDER BY lr.achv_unique_id
       ) AS rn_test
