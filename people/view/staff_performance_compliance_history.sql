...
WITH
  ...
  cert_history AS (
    ...
    FROM
      gabby.people.certification_history c
      INNER JOIN years y ON y.effective_date > c.issued_date
    WHERE
      c.valid_cert = 1
    GROUP BY
      c.employee_number,
...
FROM
  ...
  INNER JOIN years y ON y.effective_date (
    BETWEEN s.original_hire_date AND COALESCE(
      s.termination_date,
  ...
  LEFT JOIN gabby.pm.teacher_goals_overall_scores_static pm ON s.df_employee_number = pm.df_employee_number
  AND y.academic_year = pm.academic_year
  AND tm.pm_term = pm.pm_term
  LEFT JOIN gabby.people.staff_attendance_rollup a ON s.df_employee_number = a.df_employee_number
