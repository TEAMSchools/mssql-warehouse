...
FROM
  ...
  JOIN gabby.whetstone.observations_clean wo ON CAST(sr.df_employee_number AS VARCHAR(25)) = wo.teacher_internal_id
  ...
  LEFT JOIN gabby.people.staff_crosswalk_static osr ON wo.observer_internal_id = CAST(osr.df_employee_number AS VARCHAR(25))
  LEFT JOIN gabby.whetstone.observations_scores wos ON wo.observation_id = wos.observation_id
  ...
  JOIN gabby.reporting.reporting_terms rt ON wo.observed_at
  --BETWEEN rt.[start_date] AND rt.end_date 
  AND rt.identifier = 'ETR'
  AND rt.schoolid = 0
  AND rt._fivetran_deleted = 0
  LEFT JOIN gabby.pm.teacher_goals_exemption_clean_static ex ON sr.df_employee_number = ex.df_employee_number
