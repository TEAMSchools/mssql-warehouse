...
WITH
  act_composite AS (
    ...
    FROM
      ...
      INNER JOIN alumni.contact AS ktc ON (stl.contact_c = ktc.id)
    WHERE
      stl.test_type = 'ACT'
      AND stl.score_type = 'act_composite_c'
    GROUP BY
      stl.contact_c,
      ktc.school_specific_id_c
  ),
  iready AS (
    SELECT
      co.academic_year,
    ...
    FROM
      ...
      INNER JOIN iready.growth_metrics AS gm ON (
        co.student_number = gm.student_number
  ...
  sub AS (
    SELECT
      'f_and_p' AS subdomain,
      academic_year,
      schoolid,
      grade_band,
      ROUND(AVG(CAST(met_goal AS FLOAT)), 2) AS pct_met_goal,
      ROUND(
        AVG(
          CASE
            WHEN (iep_status = 'SPED') THEN CAST(met_goal AS FLOAT)
          END
        ),
        2
      ) AS pct_met_iep,
      ...
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'F') THEN CAST(met_goal AS FLOAT)
          END
        ),
        2
      ) AS pct_met_f,
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'M') THEN CAST(met_goal AS FLOAT)
          END
        ),
        2
      ) AS pct_met_m
    FROM
      (
        SELECT
          ...
          co.gender
        FROM
          lit.achieved_by_round_static AS ats
        ...
        WHERE
          ats.academic_year >= utilities.global_academic_year () - 3
    ...
    UNION ALL
    SELECT
      LOWER(grade_band) + '_gpa' AS subdomain,
      academic_year,
      schoolid,
      grade_band,
      ROUND(
        AVG(CAST(pct_met_goal AS FLOAT)),
        2
      ) AS pct_met_goal,
      ROUND(
        AVG(
          CASE
            WHEN (iep_status = 'SPED') THEN CAST(pct_met_goal AS FLOAT)
          END
        ),
        2
      ) AS pct_met_iep,
      ...
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'F') THEN CAST(pct_met_goal AS FLOAT)
          END
        ),
        2
      ) AS pct_met_f,
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'M') THEN CAST(pct_met_goal AS FLOAT)
          END
        ),
        2
      ) AS pct_met_m
    FROM
      (
        ...
        WHERE
          gpa.is_curterm = 1
          AND gpa.academic_year >= utilities.global_academic_year () - 3
    ...
    UNION ALL
    SELECT
      'i-ready_typical_' + iready_subject AS subdomain,
      academic_year,
      schoolid,
      grade_band,
      ROUND(
        AVG(CAST(is_typ_growth AS FLOAT)),
        2
      ) AS pct_met_goal,
      ROUND(
        AVG(
          CASE
            WHEN (iep_status = 'SPED') THEN CAST(is_typ_growth AS FLOAT)
          END
        ),
        2
      ) AS pct_met_iep,
      ...
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'F') THEN CAST(is_typ_growth AS FLOAT)
          END
        ),
        2
      ) AS pct_met_f,
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'M') THEN CAST(is_typ_growth AS FLOAT)
          END
        ),
        2
      ) AS pct_met_m
    ...
    UNION ALL
    SELECT
      'i-ready_stretch_' + iready_subject AS subdomain,
      academic_year,
      schoolid,
      grade_band,
      ROUND(
        AVG(CAST(is_str_growth AS FLOAT)),
        2
      ) AS pct_met_goal,
      ROUND(
        AVG(
          CASE
            WHEN (iep_status = 'SPED') THEN CAST(is_str_growth AS FLOAT)
          END
        ),
        2
      ) AS pct_met_iep,
      ...
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'F') THEN CAST(is_str_growth AS FLOAT)
          END
        ),
        2
      ) AS pct_met_f,
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'M') THEN CAST(is_str_growth AS FLOAT)
          END
        ),
        2
      ) AS pct_met_m
    ...
    UNION ALL
    SELECT
      'i-ready_on_grade_' + iready_subject AS subdomain,
      academic_year,
      schoolid,
      grade_band,
      ROUND(
        AVG(CAST(is_on_grade AS FLOAT)),
        2
      ) AS pct_met_goal,
      ROUND(
        AVG(
          CASE
            WHEN (iep_status = 'SPED') THEN CAST(is_on_grade AS FLOAT)
          END
        ),
        2
      ) AS pct_met_iep,
      ...
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'F') THEN CAST(is_on_grade AS FLOAT)
          END
        ),
        2
      ) AS pct_met_f,
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'M') THEN CAST(is_on_grade AS FLOAT)
          END
        ),
        2
      ) AS pct_met_m
    FROM
      (
        ...
        FROM
          ...
          INNER JOIN iready.diagnostic_and_instruction di ON (
    ...
    UNION ALL
    SELECT
      'suspension' AS subdomain,
      academic_year,
      schoolid,
      grade_band,
      ROUND(
        AVG(CAST(is_suspended AS FLOAT)),
        2
      ) AS pct_met_goal,
      ROUND(
        AVG(
          CASE
            WHEN (iep_status = 'SPED') THEN CAST(is_suspended AS FLOAT)
          END
        ),
        2
      ) AS pct_met_iep,
      ...
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'F') THEN CAST(is_suspended AS FLOAT)
          END
        ),
        2
      ) AS pct_met_f,
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'M') THEN CAST(is_suspended AS FLOAT)
          END
        ),
        2
      ) AS pct_met_m
    FROM
      (
        ...
        FROM
          ...
          LEFT JOIN deanslist.incidents_penalties_static ips ON (
    ...
    UNION ALL
    SELECT
      'student_attrition' AS subdomain,
      academic_year,
      schoolid,
      grade_band,
      ROUND(
        1 - ROUND(
          AVG(CAST(is_enrolled_next AS FLOAT)),
          2
        ),
        2
      ) AS pct_met_goal,
      ROUND(
        1 - ROUND(
          AVG(
            CASE
              WHEN (iep_status = 'SPED') THEN CAST(is_enrolled_next AS FLOAT)
            END
          ),
          2
        ),
      ...
      ROUND(
        1 - ROUND(
          AVG(
            CASE
              WHEN (gender = 'F') THEN CAST(is_enrolled_next AS FLOAT)
            END
          ),
          2
        ),
      ...
      ROUND(
        1 - ROUND(
          AVG(
            CASE
              WHEN (gender = 'M') THEN CAST(is_enrolled_next AS FLOAT)
            END
          ),
          2
        ),
    ...
    SELECT
      ...
      'ALL' AS grade_band,
      ROUND(
        AVG(CAST(avg_response AS FLOAT)),
        2
      ) AS pct_met_goal,
      NULL AS pct_met_iep,
    ...
    FROM
      (
        ...
        FROM
          surveys.cmo_engagement_regional_survey_detail cm
          INNER JOIN people.school_crosswalk sc ON (
        ...
        WHERE
          ...
          AND cm.campaign_academic_year >= utilities.GLOBAL_ACADEMIC_YEAR () - 3
        GROUP BY
          sc.ps_school_id,
    ...
    SELECT
      ...
      NULL AS pct_met_m
    FROM
      tableau.compliance_staff_attrition sa
      INNER JOIN people.school_crosswalk cw ON (
    ...
    UNION ALL
    SELECT
      'teacher_retention' AS subdomain,
      sa.academic_year,
      cw.ps_school_id,
      'ALL' AS grade_band,
      ROUND(
    ...
    SELECT
      ...
      NULL AS pct_met_m
    FROM
      pm.teacher_goals_lockbox_wide lb
      INNER JOIN people.employment_history_static eh ON (
    ...
    WHERE
      ...
      AND lb.pm_term = 'PM4'
      AND lb.academic_year >= utilities.GLOBAL_ACADEMIC_YEAR () - 3
    GROUP BY
      lb.academic_year,
      cw.ps_school_id
    UNION ALL
    SELECT
      'scds_student' AS subdomain,
      academic_year,
      schoolid,
      'ALL' AS grade_band,
      ROUND(AVG(CAST(met_goal AS FLOAT)), 2) AS pct_met_goal,
    ...
    FROM
      (
        SELECT
          ...
          CASE
            WHEN (
              AVG(CAST(response_value AS FLOAT)) >= 3.00
            ) THEN 1
            ELSE 0
          END AS met_goal
        FROM
    ...
    UNION ALL
    SELECT
      'state_asmt' + '_' + [subject] AS subdomain,
      academic_year,
      schoolid,
      CAST(grade_band AS NVARCHAR) AS grade_band,
      ROUND(
        AVG(CAST(is_proficient AS FLOAT)),
        2
      ) AS pct_met_goal,
      ROUND(
        AVG(
          CASE
            WHEN (iep_status = 'SPED') THEN CAST(is_proficient AS FLOAT)
          END
        ),
        2
      ) AS pct_met_iep,
      ...
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'F') THEN CAST(is_proficient AS FLOAT)
          END
        ),
        2
      ) AS pct_met_f,
      ROUND(
        AVG(
          CASE
            WHEN (gender = 'M') THEN CAST(is_proficient AS FLOAT)
          END
        ),
        2
      ) AS pct_met_m
    ...
    SELECT
      ...
      ROUND(
        AVG(
          CAST(average_response_value AS FLOAT)
        ),
        2
      ) AS pct_met_goal,
      NULL AS pct_met_iep,
...
FROM
  sub
  LEFT JOIN reporting.school_health_metric_lookup ml ON (
    sub.subdomain = ml.subdomain
    AND sub.grade_band = ml.grade_band
  )
  LEFT JOIN powerschool.schools sch ON (sub.schoolid = sch.school_number)
