WITH
  key_clean AS (
    ...
    FROM
      gabby.act.key_cleanup UNPIVOT (raw_score FOR subject IN (english, mathematics, reading, science)) u
  ),
  scaffold AS (
    SELECT
      subject,
...
FROM
  scaffold s
  LEFT OUTER JOIN key_clean a ON s.subject = a.subject
  AND s.max_raw_score = a.raw_score
  JOIN gabby.utilities.row_generator r ON r.n BETWEEN 9 AND 11  /* UPDATE */
