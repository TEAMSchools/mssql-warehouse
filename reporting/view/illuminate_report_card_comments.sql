...
WITH
  repos_union AS (
    SELECT
      sub.repository_id,
      CAST(
        sub.repository_row_id AS INT
      ) AS repository_row_id,
      sub.field_term,
  ...
  comm_unpivot AS (
    SELECT
      ...
      field_term,
      CAST(
        comment_field AS VARCHAR(125)
      ) AS comment_field,
      comment_code,
      CAST(
        RIGHT(comment_field, 1) AS INT
      ) AS comment_number
    FROM
      repos_union UNPIVOT (
...
SELECT
  ...
  cu.comment_number,
  CAST(
    cb.subject AS VARCHAR(25)
  ) AS comment_subject,
  CAST(
    cb.subcategory AS VARCHAR(125)
  ) AS subcategory,
  CAST(
    cb.comment AS VARCHAR(250)
  ) AS comment
FROM
  comm_unpivot AS cu
