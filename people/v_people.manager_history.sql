...
WITH manager_union AS (
  ...
  INNER JOIN gabby.people.employee_numbers sre
    ON mh.associate_id = sre.associate_id
   AND sre.is_active = 1
  INNER JOIN gabby.people.employee_numbers srm
    ON mh.reports_to_associate_id = srm.associate_id
   AND srm.is_active = 1
  WHERE '2021-01-01' BETWEEN CAST(mh.reports_to_effective_date AS DATE) 
                         AND COALESCE(CAST(mh.reports_to_effective_end_date AS DATE), CURRENT_TIMESTAMP)
...
SELECT employee_number
      ...
      ,COALESCE(
        ...
        ,DATEADD(DAY, -1, LEAD(reports_to_effective_date, 1) OVER(PARTITION BY position_id ORDER BY reports_to_effective_date))
