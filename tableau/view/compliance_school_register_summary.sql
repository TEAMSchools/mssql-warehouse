...
FROM
  ...
  INNER JOIN gabby.powerschool.calendar_rollup_static d ON co.schoolid = d.schoolid
  AND co.yearid = d.yearid
  AND co.track = d.track
  AND co.[db_name] = d.[db_name]
  INNER JOIN att_mem sub ON co.studentid = sub.studentid
  ...
  LEFT JOIN gabby.easyiep.njsmart_powerschool_clean_static iep ON co.student_number = iep.student_number
